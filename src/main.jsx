...
const categories = [
  ...
  { id: 'leadership', label: 'リーダーシップ', color: '#5f9e90', icon: '✦', questions: ['必要なことに気づいたら、自分から動き始められる', 'わかることや得意なことを、率先して人に教えられる', 'チームが迷っている時に、方向性を提案できる', '周りの人が動きやすくなるように声をかけられる', 'みんなの意見を聞きながら、決めるべきことを決められる'] },
...
function App() {
  ...
  const addLog = async () => {
    ...
    const newLog = { text: note.trim(), tag: goalCategory.label, date: '今日', createdAt: toDateKey(new Date()) };
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="side-bottom"><div className="streak"><Flame size={17} fill="#d2a94e" color="#d2a94e" /><span><b>{streak}日</b> 続けています</span></div>{installAvailable && <button className="help-link install-button" onClick={installApp}><Plus size={17} />スマホにインストール</button>}<div className="help-link"><CircleHelp size={17} />使い方を見る</div>{user && <button className="help-link logout-button" onClick={() => supabase.auth.signOut()}><LogOut size={17} />ログアウト</button>}</div>
    </aside>
...
function calculateStreak(logs) {
  const dates = new Set(logs.map((log) => log.createdAt || parseLogDate(log.date)).filter(Boolean));
  let streak = 0;
  const current = new Date();
  current.setHours(0, 0, 0, 0);
  while (dates.has(toDateKey(current))) {
    streak += 1;
...
function parseLogDate(value) { if (!value || value === '今日' || value === 'たった今') return null; const parsed = new Date(value); return Number.isNaN(parsed.getTime()) ? null : toDateKey(parsed); }
